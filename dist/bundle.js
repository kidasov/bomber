...
var BCELL_TYPE = exports.BCELL_TYPE = {
  STONE: 1,
  GRASS: 2
};

var BCell = function (_Phaser$Group) {
  _inherits(BCell, _Phaser$Group);
  ...
  function BCell(_ref) {
    ...
    _classCallCheck(this, BCell);

    var game = scene.game;

    var _this = _possibleConstructorReturn(this, (BCell.__proto__ || Object.getPrototypeOf(BCell)).call(this, game));
    ...
    var tile = _this.type === BCELL_TYPE.STONE ? 'bricks.png' : 'grass.png';

    _this.image = _this.game.add.sprite(x, y, 'spritesheet', tile);
    _this.image.scale.setTo(window.devicePixelRatio, window.devicePixelRatio);
    _this.image.x += _this.image.width * 0.5;
...
var BOMB_TIMER = exports.BOMB_TIMER = 5000;
var PLAYER_INVINCIBLE_TIME = exports.PLAYER_INVINCIBLE_TIME = 5000;
...
module.exports = __webpack_require__(/*! /home/bubu/www/bombermine/src/main.js */331);
...
var _pixi = __webpack_require__(/*! pixi */ 126);

var _pixi2 = _interopRequireDefault(_pixi);

var _boot = __webpack_require__(/*! ./states/boot */ 335);

var _boot2 = _interopRequireDefault(_boot);

var _preload = __webpack_require__(/*! ./states/preload */ 336);

var _preload2 = _interopRequireDefault(_preload);

var _game = __webpack_require__(/*! ./states/game */ 337);
...
var Bombermine = function (_Phaser$Game) {
  _inherits(Bombermine, _Phaser$Game);

  function Bombermine(width, height, suffix, strSuffix) {
    _classCallCheck(this, Bombermine);

    var _this = _possibleConstructorReturn(this, (Bombermine.__proto__ || Object.getPrototypeOf(Bombermine)).call(this, width, height, _phaser2.default.CANVAS, '', null));
    ...
    _this.displaySuffix = suffix;
    _this.strDisplaySuffix = strSuffix;
    return _this;
  }

  _createClass(Bombermine, [{
    key: 'getImagePathWithSuffix',
    value: function getImagePathWithSuffix(path, extension) {
      return path + this.strDisplaySuffix + extension;
    }
  }, {
    key: 'loadAssets',
    value: function loadAssets(assets) {
    ...
    key: 'loadScripts',
    value: function loadScripts(scripts) {
    ...
    key: 'getExactValue',
    value: function getExactValue(value) {
...
(function () {
  ...
  var gameWidth = DIMENSION.WIDTH * displaySuffix;
  var gameHeight = DIMENSION.HEIGHT * displaySuffix;

  _pixi2.default.DisplayObject.prototype.setPosition = function (x, y) {
    this.x = game.displaySuffix === 2 ? x * 2 : x;
    this.y = game.displaySuffix === 2 ? y * 2 : y;
  };

  _pixi2.default.DisplayObject.prototype.tryDrag = function () {
  ...
  var game = new Bombermine(windowWidth, windowHeight, displaySuffix, strDisplaySuffix);
})();
...
var _field = __webpack_require__(/*! ../game-objects/field */ 338);

var _field2 = _interopRequireDefault(_field);

var _player = __webpack_require__(/*! ../game-objects/player */ 339);

var _player2 = _interopRequireDefault(_player);

var _enemies = __webpack_require__(/*! ../game-objects/enemies */ 340);

var _enemies2 = _interopRequireDefault(_enemies);

var _bomb = __webpack_require__(/*! ../game-objects/bomb */ 342);

var _bomb2 = _interopRequireDefault(_bomb);

var _explosion = __webpack_require__(/*! ../game-objects/explosion */ 343);

var _explosion2 = _interopRequireDefault(_explosion);

var _bcell = __webpack_require__(/*! ../game-objects/bcell */ 91);

var _bcell2 = _interopRequireDefault(_bcell);

var _gameplay = __webpack_require__(/*! ../consts/gameplay */ 127);

var _bonus = __webpack_require__(/*! ../game-objects/bonus */ 344);
...
var Game = function (_Phaser$State) {
  ...
  _createClass(Game, [{
    ...
    value: function create() {
      ...
      var color = _phaser2.default.Color.getRandomColor(255, 255, 255);
      ...
      var grassCell = this.game.field.randomGrassCell;

      this.game.player = new _player2.default({
        scene: this,
        cell: grassCell,
      ...
      for (var i = 0; i < this.game.field.cells.length; i++) {
      ...
      this.game.enemies = new _enemies2.default({
        game: this.game
      });

      window.addEnemies = function (count) {
        for (var _i = 0; _i < count; _i++) {
          _this2.game.enemies.addEnemy(_this2.game.field.randomGrassCell);
          var index = _this2.game.enemies.data.length - 1;
          _this2.enemyGroup.add(_this2.game.enemies.data[index].image);
        }
      };
    ...
    key: 'dropBomb',
    value: function dropBomb(cell) {
      var bomb = new _bomb2.default({ scene: this, cell: cell });
    ...
    value: function destroyStone(cell) {
      ...
      if (chance <= 25) {
        ...
        if (chance > 0 && chance < 5) {
          ...
          bonusType = _bonus.BONUS_TYPE.SCORE_BRONZE;
        } else if (chance > 15 && chance <= 20) {
          bonusType = _bonus.BONUS_TYPE.SCORE_SILVER;
        } else if (chance > 20 && chance <= 25) {
          bonusType = _bonus.BONUS_TYPE.SCORE_GOLD;
        }
        ...
        var _game$player = this.game.player,
            ...
            speedBooster = _game$player.speedBooster,
            maxSpeedBooster = _game$player.maxSpeedBooster;


        if (bonus.type === _bonus.BONUS_TYPE.EXPLOSION_RADIUS && explosionRadius === maxExplosionRadius || bonus.type === _bonus.BONUS_TYPE.SPEED && speedBooster >= maxSpeedBooster) {
      ...
      var destroyStoneTween = this.game.add.tween(cell.image).to({ alpha: 0 }, 500, 'Linear', true);
      destroyStoneTween.onComplete.add(function () {
        cell.image.destroy();
        cell.destroy();
        cell.isDestroying = false;
        _this3.game.field.cells[row][column] = grassCell;
        _this3.game.field.computeSuccessors();
      });
    }
  }, {
    key: 'destroyPlayer',
    value: function destroyPlayer(playerSprite) {
      ...
      playerSprite.player.destroy(function () {
        var textX = (_this4.game.camera.x + _this4.game.camera.width) * 0.5;
        var textY = (_this4.game.camera.y + _this4.game.camera.height) * 0.5;
        _this4.gameOverText.x = textX;
        _this4.enemyKilledText.x = textX;
        var gameOverTextTween = _this4.game.add.tween(_this4.gameOverText).from({ y: 0 }).to({ alpha: 1, y: textY }, 500, 'Linear', true);
        gameOverTextTween.onComplete.add(function () {
          _this4.game.add.tween(_this4.enemyKilledText).from({ y: window.innerHeight }).to({ alpha: 1, y: textY + 100 }, 500, 'Linear', true);
        });
      });
    }
  }, {
    ...
    value: function createExplosion(cell) {
      ...
      setTimeout(function () {
        var explosionTween = _this5.game.add.tween(explosion).to({ alpha: 0 }, 100, 'Linear', true);
    ...
    value: function checkBonuses() {
      var _game$player2 = this.game.player,
          explosionRadius = _game$player2.explosionRadius,
          maxExplosionRadius = _game$player2.maxExplosionRadius,
          speedBooster = _game$player2.speedBooster,
          maxSpeedBooster = _game$player2.maxSpeedBooster;

      this.bonusGroup.children.forEach(function (_ref) {
    ...
    value: function update() {
      ...
      this.game.physics.arcade.overlap(this.playerGroup, this.enemyGroup, function (playerSprite, enemySprite) {
        if (!_this6.game.player.invincible) {
          _this6.destroyPlayer(playerSprite);
        }
      });
      this.game.physics.arcade.collide(this.playerGroup, this.bombGroup);
      this.game.physics.arcade.collide(this.explosionGroup, this.playerGroup, function (explosionSprite, playerSprite) {
        if (!_this6.game.player.invincible) {
          _this6.destroyPlayer(playerSprite);
        }
      });
      this.game.physics.arcade.collide(this.explosionGroup, this.enemyGroup, function (explosionSprite, enemySprite) {
        enemySprite.enemy.destroy(function () {
          _this6.enemyKilledText.text = 'Enemy killed: ' + ++_this6.enemykilled;
        });
      });
      this.game.physics.arcade.collide(this.explosionGroup, this.bombGroup, function (explosionSprite, bombSprite) {
      ...
      this.game.physics.arcade.overlap(this.playerGroup, this.bonusGroup, function (playerSprite, bonusSprite) {
        _this6.takeBonus(bonusSprite.bonus);
      });
      ...
      if (!player.isDead && !player.invincible) {
        ...
        this.game.enemies.data.forEach(function (enemy) {
          ...
          var computeAngle = function computeAngle(x, y) {
            return _this6.normalizeAngle(Math.atan2(-y + enemyY, x - enemyX) - angle);
          };
          var computeDistance = function computeDistance(x, y) {
            return Math.sqrt((x - enemyX) * (x - enemyX) + (y - enemyY) * (y - enemyY));
          };

          var blockingStone = _this6.stoneGroup.children.find(function (stone) {
          ...
          if (!blockingStone) {
            var playerRow = Math.floor(playerY / _this6.stoneGroup.children[0].height);
            var playerColumn = Math.floor(playerX / _this6.stoneGroup.children[0].width);
            var cell = _this6.game.field.cells[playerRow][playerColumn];
...
var _bcell = __webpack_require__(/*! ./bcell */ 91);
...
var Field = function (_Phaser$Group) {
  ...
  function Field(_ref) {
    var scene = _ref.scene,
        ...
        collisionGroup = _ref.collisionGroup;
    ...
    _this.cells = [];
    _this.gap = _this.game.getExactValue(0);
    _this.draw();
    return _this;
  }
  ...
  _createClass(Field, [{
    ...
    value: function draw() {
      ...
      var stoneMap = Array.from({ length: this.rows }, function () {
        return Array.from({ length: _this2.columns }, function () {
          return 1;
        });
      });
      var total = this.rows * this.columns * 0.3;

      for (var k = 0; k < total; k++) {
        var pattern = patterns[Math.floor(Math.random() * patterns.length)];
        var _x = Math.floor(Math.random() * (this.columns - pattern[0].length + 1));
        var _y = Math.floor(Math.random() * (this.rows - pattern.length + 1));
        for (var i = 0; i < pattern.length; i++) {
          for (var j = 0; j < pattern[i].length; j++) {
            var x1 = _x + j;
            var y1 = _y + i;
            if (stoneMap[y1][x1] === _bcell.BCELL_TYPE.STONE) {
    ...
    key: 'checkCell',
    value: function checkCell(row, column) {
      if (row >= 0 && row < this.rows && column >= 0 && column < this.columns) {
        var cell = this.cells[row][column];
        return cell.type !== _bcell.BCELL_TYPE.STONE && !cell.hasBomb;
      }
      return false;
    }
  }, {
    key: 'computeSuccessors',
    value: function computeSuccessors() {
    ...
    key: 'initSuccessors',
    value: function initSuccessors(cell) {
      var row = cell.row,
          column = cell.column;

      var canGoDown = this.checkCell(row + 1, column);
      var canGoUp = this.checkCell(row - 1, column);
      var canGoRight = this.checkCell(row, column + 1);
      var canGoLeft = this.checkCell(row, column - 1);
    ...
    key: 'getReachableCells',
    value: function getReachableCells(cell) {
      var _this3 = this;

      var visited = Array.from({ length: this.rows * this.columns }, function () {
        return false;
      });

      var search = function search(cell) {
        if (visited[cell.row * _this3.rows + cell.column]) {
          return [];
        }

        visited[cell.row * _this3.rows + cell.column] = true;

        return cell.successors.map(function (neighbour) {
          return search(neighbour);
    ...
    key: 'dfs',
    value: function dfs(start, finish) {
      var _this4 = this;
      ...
      var search = function search(cell) {
        if (visited[cell.row * _this4.rows + cell.column]) {
          return null;
        }
    ...
    key: 'aStar',
    value: function aStar(start, finish) {
      var _this5 = this;

      var infoMap = Array.from({ length: this.rows }, function () {
        return Array.from({ length: _this5.columns }, function () {
          return {
            f: Number.MAX_VALUE,
            g: Number.MAX_VALUE,
            h: Number.MAX_VALUE,
      ...
      while (openList.length > 0) {
        ...
        var pInfo = infoMap[p.row][p.column];
        pInfo.closed = true;

        var successors = p.successors;


        for (var j = 0; j < successors.length; j++) {
          var successor = successors[j];
          var row = successor.row,
              column = successor.column;

          var sInfo = infoMap[row][column];
          if (row === finish.row && column === finish.column) {
            var path = [finish];

            for (var cell = p; cell !== start; cell = infoMap[cell.row][cell.column].parent) {
              path.push(cell);
            }
          ...
          var gNew = pInfo.g + 1;
          var hNew = Math.abs(row - finish.row) + Math.abs(column - finish.column);
          var fNew = gNew + hNew;
    ...
    get: function get() {
      ...
      for (var i = 0; i < this.rows; i++) {
        for (var j = 0; j < this.columns; j++) {
          if (this.cells[i][j].type === _bcell.BCELL_TYPE.GRASS) {
...
var patterns = [[[2, 2], [2, 2]], [[2, 1], [2, 2]], [[2, 2], [1, 2]], [[2, 2], [2, 1]], [[1, 2], [2, 2]]];
...
var Player = function (_Phaser$Group) {
  ...
  function Player(_ref) {
    ...
    Object.assign(_this, { game: game, scene: scene, cell: cell, collisionGroup: collisionGroup });

    _this.game.add.existing(_this);
    _this.enableBody = true;
    _this.physicsBodyType = Phaser.Physics.ARCADE;

    _this.image = _this.game.add.sprite(cell.image.x, cell.image.y, 'spritesheet', 'walk-right1.png');
    _this.image.scale.setTo(window.devicePixelRatio, window.devicePixelRatio);
    _this.add(_this.image);
    ...
    _this.image.body.setCircle(radius, -radius + 0.5 * _this.image.width / _this.image.scale.x, -radius + 0.5 * _this.image.height / _this.image.scale.y);
    _this.image.anchor.setTo(0.5);

    _this.image.body.fixedRotation = true;
    _this.collisionGroup.add(_this.image);

    _this.cursors = _this.game.input.keyboard.createCursorKeys();
    game.input.keyboard.addCallbacks(_this, function (_ref2) {
      var keyCode = _ref2.keyCode;

      if (keyCode === Phaser.Keyboard.SPACEBAR) {
        if (!_this.currentCell.hasBomb) {
          _this.scene.dropBomb(_this.currentCell);
        }
      }
    }, function (_ref3) {
      ...
      if (keyCode === 37 || keyCode === 38 || keyCode === 39 || keyCode === 40) {
        _this.image.animations.stop('walk-left');
        _this.image.animations.stop('walk-right');
        _this.image.animations.stop('walk-up');
    ...
    _this.isDead = false;
    _this.speed = 100;
    _this.speedBooster = 0;
    _this.explosionRadius = 2;
    _this.maxSpeedBooster = 3;
    _this.maxExplosionRadius = 8;
    _this.invincible = true;
    _this.startScale();
    setTimeout(function () {
      _this.invincible = false;
    }, _gameplay.PLAYER_INVINCIBLE_TIME);
    return _this;
  }
  ...
  _createClass(Player, [{
    ...
    value: function addAnimations() {
      ...
      this.image.animations.add('walk-right', [1, 2, 3, 4].map(function (i) {
        return 'walk-right' + i + '.png';
      }));
      this.image.animations.add('walk-left', [1, 2, 3, 4].map(function (i) {
        return 'walk-left' + i + '.png';
      }));
      this.image.animations.add('walk-up', [1, 2, 3, 4].map(function (i) {
        return 'walk-up' + i + '.png';
      }));
      this.image.animations.add('walk-down', [1, 2, 3, 4].map(function (i) {
        return 'walk-down' + i + '.png';
      }));
    }
  }, {
    key: 'startScale',
    value: function startScale() {
      ...
      if (this.invincible) {
        var fadeOutTween = this.game.add.tween(this.image).to({ alpha: 0.5 }, 400, 'Linear', true);
        fadeOutTween.onComplete.add(function () {
          if (_this2.invincible) {
            var fadeInTween = _this2.game.add.tween(_this2.image).to({ alpha: 1 }, 400, 'Linear', true);
            fadeInTween.onComplete.add(function () {
              _this2.startScale();
            });
          } else {
            _this2.image.alpha = 1;
          }
        });
      } else {
        this.image.alpha = 1;
    ...
    value: function move() {
      ...
      if (this.isDead === false) {
        ...
        this.animationsName.forEach(function (name) {
          if (name !== animation) {
            _this3.image.animations.stop(name);
          }
        });
      }
    }
    ...
    value: function destroy(cb) {
      ...
      this.tween.onComplete.add(function () {
        _this4.image.destroy();
        _get(Player.prototype.__proto__ || Object.getPrototypeOf(Player.prototype), 'destroy', _this4).call(_this4);
      });
    }
  }, {
    key: 'currentCell',
    get: function get() {
      var cell = this.game.field.cells[0][0];
      var row = Math.floor(this.image.y / cell.image.height);
      var column = Math.floor(this.image.x / cell.image.width);
...
var _enemy = __webpack_require__(/*! ./enemy */ 341);
...
var Enemies = function (_Phaser$Group) {
  ...
  function Enemies(_ref) {
    ...
    Object.assign(_this, { game: game, amount: amount });
    _this.data = [];
    _this.game.add.existing(_this);
    return _this;
  }
  ...
  _createClass(Enemies, [{
    key: "addEnemy",
    value: function addEnemy(grassCell) {
...
var Enemy = function (_Phaser$Group) {
  ...
  function Enemy(_ref) {
    ...
    var _this = _possibleConstructorReturn(this, (Enemy.__proto__ || Object.getPrototypeOf(Enemy)).call(this, game));

    Object.assign(_this, { game: game, cell: cell, collisionGroup: collisionGroup });
  ...
  _createClass(Enemy, [{
    ...
    key: 'isSameCoordinate',
    value: function isSameCoordinate(obj, obj2) {
      return obj.x === obj2.x && obj.y === obj2.y;
    }
  }, {
    key: 'chooseTarget',
    value: function chooseTarget(origin) {
      var _this2 = this;
      ...
      var next = function next() {
        if (i >= path.length || _this2.playerTarget !== null && !_this2.isSameCoordinate(_this2.playerTarget.image, _this2.target.image)) {
          _this2.chooseTarget(path[i - 1]);
          return;
        }
        ...
        if (nextTarget.row - prevTarget.row === 1) {
          _this2.image.animations.play('pacman-down', 10, true);
        } else if (nextTarget.row - prevTarget.row === -1) {
          _this2.image.animations.play('pacman-up', 10, true);
        } else if (nextTarget.column - prevTarget.column === 1) {
          _this2.image.animations.play('pacman-right', 10, true);
        } else if (nextTarget.column - prevTarget.column === -1) {
          _this2.image.animations.play('pacman-left', 10, true);
        }

        _this2.tween = _this2.game.add.tween(_this2.image).to({ x: nextTarget.image.x, y: nextTarget.image.y }, speed);

        _this2.tween.onComplete.add(next);
        _this2.tween.start();
      };
...
var Bomb = function (_Phaser$Group) {
  _inherits(Bomb, _Phaser$Group);
  ...
  function Bomb(_ref) {
    ...
    var _this = _possibleConstructorReturn(this, (Bomb.__proto__ || Object.getPrototypeOf(Bomb)).call(this, game));
    ...
    _this.image = _this.game.add.sprite(image.x, image.y, 'spritesheet', 'bomb1.png');
    _this.image.anchor.setTo(0.5);
    _this.image.animations.add('bomb', [1, 2, 3, 4].map(function (i) {
      return 'bomb' + i + '.png';
    }));
    _this.game.physics.enable(_this.image, _phaser2.default.Physics.ARCADE);
    _this.image.body.immovable = true;
    ...
    _this.add(_this.image);
    _this.game.add.existing(_this);
    _this.image.animations.play('bomb', 10, true);

    _this.cell.hasBomb = true;

    _this.bombTimer = setTimeout(function () {
      _this.explode();
    }, window.bombTimer);
...
var Explosion = function (_Phaser$Group) {
  _inherits(Explosion, _Phaser$Group);

  function Explosion(_ref) {
    var scene = _ref.scene,
        cell = _ref.cell,
        radius = _ref.radius;
    ...
    _this.centerImage = _this.game.add.sprite(cell.image.x, cell.image.y, 'spritesheet', 'explosion-center.png');
    _this.centerImage.anchor.setTo(0.5);
    _this.game.physics.enable(_this.centerImage, Phaser.Physics.ARCADE);
    _this.centerImage.body.immovable = true;
    _this.centerImage.body.setSize(0.6 * _this.centerImage.width, 0.6 * _this.centerImage.height, 0.2 * _this.centerImage.width, 0.2 * _this.centerImage.height);
    _this.centerImage.scale.setTo(window.devicePixelRatio, window.devicePixelRatio);
    _this.add(_this.centerImage);
    ...
    for (var i = 1; i <= radius; i++) {
      if (cell.column - i >= 0 && hasLeft) {
        var leftCell = _this.game.field.cells[cell.row][cell.column - i];
        var leftMiddle = _this.game.add.sprite(cell.image.x - cell.image.width * i, cell.image.y, 'spritesheet', i === radius ? 'explosion-end.png' : 'explosion-middle.png');
        leftMiddle.anchor.setTo(0.5);
        leftMiddle.angle = 180;
        _this.game.physics.enable(leftMiddle, Phaser.Physics.ARCADE);
        ...
        _this.add(leftMiddle);

        if (leftCell.type !== _bcell.BCELL_TYPE.GRASS) {
          hasLeft = false;
          scene.destroyStone(leftCell);
        }
      }

      if (cell.column + i < _this.game.field.columns && hasRight) {
        var rightCell = _this.game.field.cells[cell.row][cell.column + i];
        var rightMiddle = _this.game.add.sprite(cell.image.x + cell.image.width * i, cell.image.y, 'spritesheet', i === radius ? 'explosion-end.png' : 'explosion-middle.png');
        rightMiddle.anchor.setTo(0.5);
        _this.game.physics.enable(rightMiddle, Phaser.Physics.ARCADE);
        ...
        _this.add(rightMiddle);

        if (rightCell.type !== _bcell.BCELL_TYPE.GRASS) {
          hasRight = false;
      ...
      if (cell.row - i >= 0 && hasTop) {
        var topCell = _this.game.field.cells[cell.row - i][cell.column];
        var topMiddle = _this.game.add.sprite(cell.image.x, cell.image.y - cell.image.height * i, 'spritesheet', i === radius ? 'explosion-end.png' : 'explosion-middle.png');
        topMiddle.anchor.setTo(0.5);
        topMiddle.angle = -90;
        _this.game.physics.enable(topMiddle, Phaser.Physics.ARCADE);
        ...
        if (topCell.type !== _bcell.BCELL_TYPE.GRASS) {
          hasTop = false;
          scene.destroyStone(topCell);
        }
      }

      if (cell.row + i < _this.game.field.rows && hasBottom) {
        var bottomCell = _this.game.field.cells[cell.row + i][cell.column];
        var bottomMiddle = _this.game.add.sprite(cell.image.x, cell.image.y + cell.image.height * i, 'spritesheet', i === radius ? 'explosion-end.png' : 'explosion-middle.png');
        bottomMiddle.anchor.setTo(0.5);
        bottomMiddle.angle = 90;
        _this.game.physics.enable(bottomMiddle, Phaser.Physics.ARCADE);
        ...
        _this.add(bottomMiddle);

        if (bottomCell.type !== _bcell.BCELL_TYPE.GRASS) {
          hasBottom = false;
...
var Bonus = function (_Phaser$Group) {
  ...
  function Bonus(_ref) {
    ...
    switch (_this.type) {
      ...
      case BONUS_TYPE.SCORE_BRONZE:
        bonusImg = 'bonus-score2.png';break;
      case BONUS_TYPE.SCORE_SILVER:
        bonusImg = 'bonus-score3.png';break;
      case BONUS_TYPE.SCORE_GOLD:
        bonusImg = 'bonus-score1.png';break;
    }
    ...
    _this.image.body.setSize(0.5 * _this.image.width, 0.5 * _this.image.height, 0.25 * _this.image.width, 0.25 * _this.image.height);
