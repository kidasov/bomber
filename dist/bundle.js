...
module.exports = __webpack_require__(/*! /home/bubu/www/bombermine/src/main.js */329);
...
var _pixi = __webpack_require__(/*! pixi */ 125);

var _pixi2 = _interopRequireDefault(_pixi);

var _boot = __webpack_require__(/*! ./states/boot */ 333);

var _boot2 = _interopRequireDefault(_boot);

var _preload = __webpack_require__(/*! ./states/preload */ 334);
...
var _field = __webpack_require__(/*! ../game-objects/field */ 336);
  ...
  _createClass(Game, [{
    ...
    value: function create() {
      ...
      for (var _i = 0; _i < 3; _i++) {
        ...
        this.enemyGroup.add(this.game.enemies.data[_i].image);
        var enemyBody = this.game.enemies.data[_i].image.body;
    ...
    value: function render() {
      ...
      this.game.enemies.data.forEach(function (enemy) {
        ...
        var computeAngle = function computeAngle(x, y) {
          return _this2.normalizeAngle(Math.atan2(-y + enemyY, x - enemyX) - angle);
        };
        var computeDistance = function computeDistance(x, y) {
          return Math.sqrt((x - enemyX) * (x - enemyX) + (y - enemyY) * (y - enemyY));
        };

        var blockingStone = _this2.stoneGroup.children.find(function (stone) {
        ...
        if (blockingStone) {
          ...
          var playerRow = Math.floor(playerY / _this2.stoneGroup.children[0].height);
          var playerColumn = Math.floor(playerX / _this2.stoneGroup.children[0].width);
          var cell = _this2.game.field.cells[playerRow][playerColumn];
...
var _bcell = __webpack_require__(/*! ./bcell */ 337);
...
var Field = function (_Phaser$Group) {
  ...
  function Field(_ref) {
    var game = _ref.game,
        ...
        playerCollisionGroup = _ref.playerCollisionGroup;
  ...
  _createClass(Field, [{
    ...
    value: function draw() {
      ...
      for (var _i2 = 0; _i2 < this.rows; _i2++) {
        for (var _j2 = 0; _j2 < this.columns; _j2++) {
          this.initSuccessors(this.cells[_i2][_j2], stoneMap);
        }
      }
    }
  }, {
    key: 'initSuccessors',
    value: function initSuccessors(cell, stoneMap) {
      var row = cell.row,
          column = cell.column;

      var canGoDown = row + 1 < this.rows && stoneMap[row + 1][column] !== _bcell.BCELL_TYPE.STONE;
      var canGoUp = row - 1 >= 0 && stoneMap[row - 1][column] !== _bcell.BCELL_TYPE.STONE;
      var canGoRight = column + 1 < this.columns && stoneMap[row][column + 1] !== _bcell.BCELL_TYPE.STONE;
      var canGoLeft = column - 1 >= 0 && stoneMap[row][column - 1] !== _bcell.BCELL_TYPE.STONE;
      ...
      if (canGoDown && canGoLeft && stoneMap[row + 1][column - 1] !== _bcell.BCELL_TYPE.STONE) {
        cell.successors.push(this.cells[row + 1][column - 1]);
      }

      if (canGoDown && canGoRight && stoneMap[row + 1][column + 1] !== _bcell.BCELL_TYPE.STONE) {
        cell.successors.push(this.cells[row + 1][column + 1]);
      }

      if (canGoUp && canGoLeft && stoneMap[row - 1][column - 1] !== _bcell.BCELL_TYPE.STONE) {
        cell.successors.push(this.cells[row - 1][column - 1]);
      }

      if (canGoUp && canGoRight && stoneMap[row - 1][column + 1] !== _bcell.BCELL_TYPE.STONE) {
        cell.successors.push(this.cells[row - 1][column + 1]);
      }
    }
  }, {
    ...
    key: 'isDestinationVisible',
    value: function isDestinationVisible(source, destination, radius) {
      var dx = source.x - destination.x;
      var dy = source.y - destination.y;
      var distance = Math.sqrt(dy * dy + dx * dx);
      var offsetX = radius * dy / distance;
      var offsetY = radius * dx / distance;
      var pointTwo = { x: source.x + offsetX, y: source.y + offsetY };
      var pointThree = { x: source.x - offsetX, y: source.y - offsetY };
    }
  }, {
    key: 'aStar',
...
var Player = function (_Phaser$Group) {
  ...
  function Player(_ref) {
    ...
    game.input.keyboard.addCallbacks(_this, null, function (_ref2) {
      var keyCode = _ref2.keyCode;

      if (keyCode === 37 || keyCode === 38 || keyCode === 39 || keyCode === 40) {
        _this.image.body.velocity.x = 0;
        _this.image.body.velocity.y = 0;
  ...
  _createClass(Enemy, [{
    ...
    value: function chooseTarget(origin) {
      ...
      var next = function next() {
        ...
        var tween = _this2.game.add.tween(_this2.image).to({ x: nextTarget.image.x, y: nextTarget.image.y }, speed);
